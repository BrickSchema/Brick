...
def reason_brick(g):
    ...
    for (classname, groupname), props in grouped_properties.items():
        q = "INSERT {\n"
        q += "\n".join([f"\t ?inst <{prop}> <{obj}> ." for prop, obj in props])
        q += "\n} WHERE {\n"
        q += "\n".join([f"\t ?inst rdf:type <{classname}> ."])
        q += "\n}"
        g.update(q)
...
def reason_agraph(g):
    ...
    check_error(
        agraph.exec_run("/app/agraph/bin/agload test /opt/input.ttl", user="agraph")
    )
    check_error(
        agraph.exec_run("/app/agraph/bin/agmaterialize test --rule all", user="agraph")
    )
    check_error(
        agraph.exec_run(
            "/app/agraph/bin/agexport -o turtle test /opt/output.ttl", user="agraph"
        )
    )
